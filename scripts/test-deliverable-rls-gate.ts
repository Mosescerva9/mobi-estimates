...
test("deliverables table writes are restricted to admin/owner users", () => {
  const insertPolicy = policyBody("deliverables_insert_admin");
  assert(insertPolicy.includes("for insert"), "insert policy must cover metadata inserts");
  assert(insertPolicy.includes("public.is_admin()"), "metadata inserts must require admin");
  assert(!insertPolicy.includes("public.is_staff()"), "metadata inserts must not allow estimator/reviewer staff");

  const updatePolicy = policyBody("deliverables_update_admin");
  assert(updatePolicy.includes("for update"), "update policy must cover metadata mutations");
  assert(updatePolicy.includes("public.is_admin()"), "metadata updates must require admin");
  assert(!updatePolicy.includes("public.is_staff()"), "metadata updates must not allow estimator/reviewer staff");
});

test("deliverables storage object writes are restricted to admin/owner users", () => {
  for (const policyName of ["\"deliverables_insert\"", "\"deliverables_update\"", "\"deliverables_delete\""]) {
    ...
    assert(body.includes("public.is_admin()"), `${policyName} must require admin`);
