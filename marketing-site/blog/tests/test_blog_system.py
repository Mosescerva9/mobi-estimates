...
def main():
    ...
    fail_if('internal process commentary visible', 'not navigation' in a.lower() or 'not navigation' in pub.lower(), failures)
    ...
    result={'status':'ok' if not failures else 'failed','failures':failures,'checked_pages':[str(p) for p in PAGES],'canonical_articles':[m['id'] for m in metas],'published_articles':[m['id'] for m in published]}
