...
export const CAPABILITY_STAGES = ["planned", "source", "staging", "production", "accuracy_validated"] as const;
...
const DELIVERY_GRADE_STAGES = new Set<CapabilityStage>(["production", "accuracy_validated"]);

export function isDeliveryGradeCapability(stage: CapabilityStage | string | null | undefined): stage is CapabilityStage {
  return stage === "production" || stage === "accuracy_validated";
}
 ...
 * a marketing claim. It intentionally distinguishes source/staging/admin workflow
 * evidence from production/accuracy-validated delivery support so UI, docs, or
 * tests cannot be mistaken for final construction-estimate readiness.
 */
export const PORTAL_CAPABILITY_REGISTRY = {
  ...
  supported_trade_accuracy: {
    ...
    deliveryGrade: false,
    customerVisible: true,
    summary: "No portal-supported trade lane is production/accuracy validated for autonomous customer delivery.",
