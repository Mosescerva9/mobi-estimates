...
begin
  ...
  if p_packet_manifest is null or jsonb_typeof(p_packet_manifest) <> 'object' then
    return jsonb_build_object('ok', false, 'reason', 'manifest_not_object');
  end if;
  if not (p_packet_manifest ? 'schema_version')
     or jsonb_typeof(p_packet_manifest->'schema_version') <> 'string' then
  ...
  if not (p_packet_manifest ? 'packet') or jsonb_typeof(p_packet_manifest->'packet') <> 'object' then
    return jsonb_build_object('ok', false, 'reason', 'packet_not_object');
  end if;
  v_packet := p_packet_manifest->'packet';
  if not (v_packet ? 'sha256') or jsonb_typeof(v_packet->'sha256') <> 'string'
     or (v_packet->>'sha256') !~ '^[0-9a-f]{64}$' then
    return jsonb_build_object('ok', false, 'reason', 'invalid_packet_sha256');
  end if;
  if jsonb_typeof(v_packet->'page_count') <> 'number'
     or jsonb_typeof(v_packet->'source_count') <> 'number'
     or jsonb_typeof(v_packet->'bytes') <> 'number' then
  ...
  if v_page_count <= 0 or v_source_count <= 0 then
    return jsonb_build_object('ok', false, 'reason', 'nonpositive_packet_counts');
  end if;

  -- Engine link must agree with the manifest's declared page count.
  ...
  if not (p_packet_manifest ? 'sources') or jsonb_typeof(p_packet_manifest->'sources') <> 'array' then
  ...
  if exists (
    ...
    where jsonb_typeof(e) <> 'object'
       or jsonb_typeof(e->'order') <> 'number'
       or jsonb_typeof(e->'source_bytes') <> 'number'
       or jsonb_typeof(e->'source_page_count') <> 'number'
       or jsonb_typeof(e->'combined_page_start') <> 'number'
       or jsonb_typeof(e->'combined_page_end') <> 'number'
       or jsonb_typeof(e->'project_file_id') <> 'string'
       or jsonb_typeof(e->'storage_path') <> 'string'
       or jsonb_typeof(e->'original_filename') <> 'string'
       or jsonb_typeof(e->'source_sha256') <> 'string'
  ...
  if v_stats.n <> v_source_count
     or v_stats.dord <> v_source_count
     or v_stats.mino <> 0
     or v_stats.maxo <> v_source_count - 1
     or v_stats.dpf <> v_source_count
     or v_stats.dsp <> v_source_count then
    return jsonb_build_object('ok', false, 'reason', 'invalid_source_orders_or_duplicates');
  end if;
  if v_stats.sump <> v_page_count then
  ...
  if v_accepted_count <> v_source_count or v_match_count <> v_source_count then
