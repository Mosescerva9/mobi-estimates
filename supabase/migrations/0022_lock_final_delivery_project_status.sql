...
-- that imply customer final delivery (delivered/revised/approved). These RLS policies
...
create policy status_history_insert_staff on public.project_status_history
  for insert with check (
    public.is_staff()
    and to_status not in ('delivered', 'revised', 'approved')
  );
...
as $$
begin
  if new.status in ('delivered', 'revised', 'approved') then
...
as $$
begin
  if new.to_status in ('delivered', 'revised', 'approved') then
