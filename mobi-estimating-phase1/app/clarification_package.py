...
def _candidate_id(entry: dict[str, Any], index: int) -> str:
    ...
    digest = hashlib.sha256(f"{index}|{raw}".encode("utf-8")).hexdigest()[:12]
...
def build_clarification_package(project_id: UUID, *, register: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    candidates = [_candidate_from_entry(entry, idx) for idx, entry in enumerate(entries)]
